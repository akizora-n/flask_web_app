...
def upload_image():
    ...
    if form.validate_on_submit():
        ...
        # ファイルのファイル名と拡張子を取得し、ファイル名をuuidに変更する
...
def draw_lines(c1, c2, result_image, line, color) -> cv2:
    # 四角形の枠線を画像に追記
    cv2.rectangle(result_image, c1, c2, color, thickness=line)
    return cv2


def draw_texts(result_image, line, c1, cv2: cv2, color, labels, label) -> cv2:
    # 検知したテキストラベルを画像に追記
...
def save_detected_image_tags(user_image, tags: set, detected_image_file_name):
    # 検知後画像の保存先パスをDBに保存する
