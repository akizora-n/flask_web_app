...
def test_index(client):
    rv = client.get("/")
    # rv.dataにはアクセスした結果のHTMLが格納されている
...
def test_upload_signup_get(client):
    signup(client, "admin", "flask@example.com", "password")
...
# 画像アップロード後に検出ボタンをクリックしたときのテスト
def test_detector_no_user_image(client):
    signup(client, "admin", "flaskbook@example.com", "password")
    upload_image(client, "detector/testdata/test_valid_image.jpg")
    # 存在しないIDを指定する
    rv = client.post("/detect/notexisted", follow_redirects=True)
